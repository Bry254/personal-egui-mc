...
fn gen_cmd(version: &String, addons: Option<String>, share_path: bool) -> Vec<String> {
    let mut r = vec!["mcpelauncher-client".to_string(), "-dg".to_string(), version.clone()];
    if !share_path {
        r.push("-dd".to_string());
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if &self.apk != &"".to_string() && &self.name != &"".to_string() {
                if ui.button(format!("Instalar \"{}\" apk", self.name)).clicked() {
                    ...
                    if Command::new("mcpelauncher-extract").args([&self.apk, &targetpath]).output().unwrap().status.success() {
